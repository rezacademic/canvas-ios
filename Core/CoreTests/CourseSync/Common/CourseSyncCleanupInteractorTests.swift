...
class CourseSyncCleanupInteractorTests: XCTestCase {
    ...
    func testSharedOfflineDirectoryForUserDeleted() throws {
        ...
        let dbURL = URL.Directories.databaseURL(appGroup: "group.dev.shadowing.icanvas", session: mockSession)
        ...
        let anotherDbURL = URL.Directories.databaseURL(appGroup: "group.dev.shadowing.icanvas", session: anotherMockSession)
        try write("test", to: anotherDbURL)
        XCTAssertTrue(FileManager.default.fileExists(atPath: anotherDbURL.path))

        // MARK: - WHEN
        let testee = CourseSyncCleanupInteractor(appGroup: "group.dev.shadowing.icanvas", session: mockSession)
